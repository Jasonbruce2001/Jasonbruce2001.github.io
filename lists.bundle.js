  ...
  return _createClass(List, [{
    ...
    value: function addEventListeners() {
      ...
      lists.forEach(function (list, i) {
        lists[i].onclick = _this.selectList.bind(_this, i);
      });
    }
  }, {
    key: "addList",
    value: function addList() {
      this.visualAddPress();
      var NEW_LIST = {
        listName: "List ".concat(this.lists.length + 1),
        numMovie: 0
      };
    ...
    key: "selectList",
    value: function selectList(index) {
      //event.preventDefault();

      try {
        this.deselectLists();
        this.renameInput = document.getElementById("renameInput");
        document.getElementById("list".concat(index)).style.backgroundColor = "LightGray";
    ...
    value: function renameList() {
      if (this.selectedIndex != -1) {
        var list = this.lists[this.selectedIndex];
    ...
    key: "deselectLists",
    value: function deselectLists() {
      try {
        for (var i = 0; i < this.lists.length; i++) {
          document.getElementById("list".concat(i)).style.backgroundColor = "white";
        }
    ...
    key: "renderStoredLists",
    value: function renderStoredLists() {
      var html = '';
      for (var i = 0; i < this.lists.length; i++) {
      ...
      for (var _i = 0; _i < this.lists.length; _i++) {
        document.getElementById("deleteList".concat(_i)).onclick = this.removeStoredList.bind(this, _i);
        document.getElementById("list".concat(_i)).onclick = this.selectList.bind(this, _i);
      }
    }

    //renders html for a single stored list
  }, {
    key: "renderStoredList",
    value: function renderStoredList(listNum) {
      var list = this.lists[listNum];
      return "<div class=\"listItem list\" id=list".concat(listNum, ">\n                    <h4>- ").concat(list[0].listName, " <i class=\"bi bi-dash-square\" style=\"float:right; margin-right: 15px;\" id=\"deleteList").concat(listNum, "\"></i></h4>\n                </div>");
    }
  }, {
    key: "renderListContents",
    value: function renderListContents(list) {
      var html = "";
      console.log(list);
      if (list.length == 1) {
    ...
    value: function renderListContent(list, index) {
      return "<div class=\"row resultItem\" id=\"\">\n                    <div class=\"col-2\"><img src=\"".concat(IMAGE_URL).concat(list[index].path, "\" alt=\"Movie Poster\" style=\"width: 90%; height: 90%; margin-top: 7%;\"></div> \n                    <div class=\"col\"><p><b>Title:</b> ").concat(list[index].title, " <br />\n                        <b>Rating:</b> ").concat(list[index].rating, " \n                        <br /><br /><b>Comments: </b> ").concat(list[index].comments, "\n                    </div>\n                </div>");
    }
  }, {
    key: "removeStoredList",
    value: function removeStoredList(index, event) {
      this.deselectLists();
      event.preventDefault();
    ...
    value: function visualAddPress() {
      ...
      setTimeout(function () {
        addListIcon.classList.remove("bi-plus-square-fill");
        addListIcon.classList.add("bi-plus-square");
      }, 250);
    }
  }, {
    key: "visualMinusPress",
    value: function visualMinusPress(index) {
      var target = document.getElementById("deleteList".concat(index));
      console.log(target);
      ...
      //call setTimeout, contains arrow function for switch icon classes back to normal state, calls after 250ms 
      setTimeout(function () {
        target.classList.remove("bi-dash-square-fill");
        target.classList.add("bi-dash-square");
      }, 250);
    }
  }]);
}();
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["vendors-node_modules_bootstrap_dist_js_bootstrap_esm_js-node_modules_bootstrap_dist_css_boots-bd9352","vendors-node_modules_regenerator-runtime_runtime_js"], () => (__webpack_require__("./src/js/lists.js")))
